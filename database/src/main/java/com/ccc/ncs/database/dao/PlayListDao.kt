package com.ccc.ncs.database.dao

import androidx.room.Dao
import androidx.room.Insert
import androidx.room.OnConflictStrategy
import androidx.room.Query
import androidx.room.Transaction
import androidx.room.Update
import com.ccc.ncs.database.model.PlayListEntity
import com.ccc.ncs.database.model.reference.PlayListMusicCrossRef
import com.ccc.ncs.database.model.relation.PlayListWithMusics
import kotlinx.coroutines.flow.Flow
import java.util.UUID

@Dao
abstract class PlayListDao {
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    abstract suspend fun insertPlayList(playList: PlayListEntity): Long

    @Transaction
    @Query("SELECT * FROM play_list WHERE id = :id")
    abstract fun getPlayList(id: UUID): Flow<PlayListWithMusics?>

    @Transaction
    @Query("SELECT * FROM play_list WHERE isUserCreated = 0")
    abstract fun getAutoGeneratedPlaylist(): Flow<PlayListWithMusics?>

    @Transaction
    @Query("SELECT * FROM play_list WHERE rowid = :rowId")
    abstract fun getPlayListByRowId(rowId: Long): Flow<PlayListWithMusics?>

    @Transaction
    @Query("SELECT * FROM play_list ORDER BY rowid")
    abstract fun getAllPlayList(): Flow<List<PlayListWithMusics>>

    @Insert(onConflict = OnConflictStrategy.REPLACE)
    abstract suspend fun linkMusicToPlayList(crossRef: List<PlayListMusicCrossRef>)

    @Query("DELETE FROM play_list_music_cross_ref WHERE playListId = :playListId")
    abstract suspend fun unLinkAllMusic(playListId: UUID)

    @Query("DELETE FROM play_list_music_cross_ref WHERE musicId = :musicId")
    abstract suspend fun unLinkMusic(musicId: UUID)

    @Transaction
    open suspend fun setMusics(playlistId: UUID, musicIds: List<UUID>) {
        unLinkAllMusic(playlistId)
        linkMusicToPlayList(musicIds.distinct().map {
            PlayListMusicCrossRef(
                playListId = playlistId,
                musicId = it
            )
        })
    }

    @Update
    abstract suspend fun updatePlayList(playList: PlayListEntity)

    @Query("DELETE FROM play_list WHERE id = :id")
    abstract suspend fun deletePlayList(id: UUID)
}